...
bool Raytracer::parseInput(const string& fileName) {
    ...
    for(size_t i = 0;i < lines.size();i++){
        ...
        if (tokens[0] == "T"){            
            ...
            vec3  t_normal_ = vec3(stof(tokens[10]), stof(tokens[11]), stof(tokens[12]));         
            
            

            t_buffer.push_back(new Triangle(p0, p1, p2, tmp_m, t_normal_));
            //o_buffer.push_back(new Triangle(p0, p1, p2, tmp_m, t_normal_));
    ...
    o_buffer.push_back(new Mesh(t_buffer));
    cam = new Camera(origin, unit_view_d, resolution, fov);
    t_buffer.clear();

    
    //cout << tree->get_bounding_box().bl[0] << "," <<tree->get_bounding_box().bl[1]<< "," << tree->get_bounding_box().bl[2] << endl;
    //cout << tree->get_bounding_box().tr[0] << "," <<tree->get_bounding_box().tr[1]<< "," << tree->get_bounding_box().tr[2] << endl;

}

bool Raytracer::findFirstObject(Ray ray, Intersection* nearestIntersection){
    bool found = false;
...
vec3 Raytracer::trace(Ray ray, int reflect_count){
    ...
    if (!findFirstObject(ray, &intersection)){
    ...
    */
    ...
    for (int i = 0 ; i < l_buffer.size() ; i++) {
        ...
        bool found = findFirstObject(lightray, &light_intersection);
        ...
        if (found == false || (light_intersection.intersect_Point - intersection.intersect_Point ).length() > 0.0001 ) //if in shadow
            ...
            //cout << intersection.unit_normal[0] << ","<< intersection.unit_normal[1] << ","<< intersection.unit_normal[2] << endl;
    ...
    vec3 reflection_color = trace(Ray(intersection.intersect_Point + (offset * reflected), reflected.normalize()) , ++reflect_count);
